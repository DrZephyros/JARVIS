...
- Use play_media to play music, songs, playlists, or watch videos (<media_service>). This automatically triggers the agent to find and click the correct media.
...
class Jarvis:
    ...
    def _try_fast_path(self, text: str):
            ...
            if "spotify" in t or not any(w in t for w in ["youtube", "on youtube"]):
                ...
                if wants_liked:
                    ...
                    if query:
                        ...
                        self._queue_agent(f"Find the Play button for '{query}' and click it to start the music.")
                        return (f"Searching Spotify for {query}, Sir.", True)
                    else:
                        subprocess.Popen("start spotify:", shell=True)
                        return ("Opening Spotify now, Sir.", True)
            
            # YouTube music fallback
            if "youtube" in t or "on youtube" in t:
                m = re.search(r'play\s+(.*?)(?:\s+on\s+youtube|\s*$)', t)
                query = m.group(1).strip() if m else "music"
                url = f"https://www.youtube.com/results?search_query={urllib.parse.quote(query)}"
                self._open_urls_in_chrome([url])
                self._queue_agent(f"Find the video for '{query}' and click it to play.")
                return (f"Searching YouTube for {query}, Sir.", True)

        # ── Media transport controls (ONLY when no app/song specified) ───
        # These only fire for simple commands like "pause", "next track"
        if not is_music_request:
            if "pause" in t and any(w in t for w in ["music", "spotify", "media", "video", "it", "this", "that"]):
                pyautogui.press("playpause")
                return ("Paused, Sir.", True)
            if t in ("pause", "resume", "play", "unpause"):
                pyautogui.press("playpause")
                return ("Done, Sir.", True)

        if "next track" in t or "skip song" in t or "next song" in t or "skip track" in t:
            pyautogui.press("nexttrack")
            return ("Skipping track, Sir.", True)
        if "previous track" in t or "previous song" in t:
            pyautogui.press("prevtrack")
            return ("Playing previous track, Sir.", True)
        ...
        # Only fast-path pure "open X" commands (short, no follow-up work).
        # "open excel and create a spreadsheet" should fall through to the agent.
        has_launch_verb = any(w in t for w in ["open", "launch", "start", "run"])
        has_followup = any(w in t for w in [" and ", " then ", " to create", " to make", " to write", " to edit"])
        if has_launch_verb and not has_followup and len(t.split()) <= 8:
            for keyword, cmd in app_map.items():
                if keyword in t:
                    try:
                        self._launch_app(keyword)
                        return (f"Opening {keyword.title()} now, Sir.", True)
                    except Exception as e:
                        logger.error("Fast-path app launch failed: %s", e)
                        return (None, False)

        # ── Browser actions ──────────────────────────────────────────────
        if any(k in t for k in ["refresh the page", "refresh page", "reload the page", "reload page", "refresh this"]):
            pyautogui.hotkey("ctrl", "r")
            return ("Page refreshed, Sir.", True)

        if "new tab" in t:
            pyautogui.hotkey("ctrl", "t")
            return ("New tab opened, Sir.", True)

        if "close tab" in t or "close this tab" in t:
            pyautogui.hotkey("ctrl", "w")
            return ("Tab closed, Sir.", True)

        # ── URL navigation ───────────────────────────────────────────────
        url_match = re.search(r'(?:go to|open|navigate to|visit)\s+((?:https?://)?(?:www\.)?[\w.-]+\.(?:com|org|net|io|dev|edu|gov|co|ai|app)(?:/\S*)?)', t)
        if url_match:
            url = url_match.group(1)
            url = self._normalize_url(url)
            self._open_urls_in_chrome([url])
            return (f"Navigating to {url}, Sir.", True)

        # Google search
        search_match = re.search(r'(?:google|search|search for|look up)\s+(.+)', t)
        if search_match and "on my screen" not in t:
            query = search_match.group(1).strip().strip('"').strip("'")
            url = f"https://www.google.com/search?q={urllib.parse.quote(query)}"
            self._open_urls_in_chrome([url])
            return (f"Searching for {query}, Sir.", True)
